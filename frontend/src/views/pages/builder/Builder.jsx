...
const Builder = () => {
  ...
  return (
    <div className="w-full h-screen">
      ...
      {loading ? (
        <div>
          <MetaData title={`${noteId} | Summit Scholars Hub`} description={`${noteId} | Summit Scholars Hub`} />
          <InitialLoader loading={true} />
        </div>
      ) : templateSections ? (
        <>
          <MetaData
            title={`${singleTemplateData?.title} | Summit Scholars Hub`}
            description={`${singleTemplateData?.description} | Summit Scholars Hub`}
          />
          <BuilderView />
        </>
      ) : (
