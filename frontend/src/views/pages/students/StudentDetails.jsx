...
const StudentDetails = () => {
  ...
  const { singleStudentDetail } = useSelector((s) => s.studentState);
  ...
  const breadCrumbs = useMemo(() => {
    const current = menuItems.find((item) => item.id === pageType);
      ...
      <div className="flex gap-6 mt-4 h-[85vh]">
        <div className="flex flex-col gap-6 w-4/5">
          <div className="max-h-[85vh] overflow-y-auto pr-2">
            {pageType === 'student-fees' ? (
              <StudentFees
                studentId={studentId}
                studentName={singleStudentDetail?.name || ''}
              />
            ) : (
              <ComponentRender />
            )}
            ...
            <div className="max-h-[calc(100vh-100px)] overflow-y-auto pr-2">
              <nav className="flex flex-col space-y-1">
                {menuItems.map((item) => (
                  <Button
                    key={item.id}
                    variant={pageType === item.id ? 'secondary' : 'ghost'}
