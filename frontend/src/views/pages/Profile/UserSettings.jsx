...
const menuItemsConstant = [
  {
    id: 'my-profile',
    label: 'My Profile',
    breadCrumbs: [{ label: 'Settings', href: null }, { label: 'Profile', href: null }],
  },
  {
    id: 'change-password',
    label: 'Change Password',
    breadCrumbs: [{ label: 'Settings', href: null }, { label: 'Change Password', href: null }],
  },
];
...
const Setting = () => {
  ...
  useEffect(() => {
    // Only ADMIN gets the Google Integrations tab
    if (profileDetails?.role === ADMIN) {
      setMenuItems((prev) => {
        const alreadyAdded = prev.some((item) => item.id === 'admin-integrations');
  ...
  const breadCrumbs = useMemo(() => {
    const current = menuItems.find((item) => item.id === settingId);
