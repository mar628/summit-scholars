...
const createNewLiveClassController = async (req, res, next) => {
  try {
    ...
    // Find enrollment - try with batch first, then fallback to any enrollment
    let enrollmentRecord = await enrollmentProgressModel.findOne({
      studentId: studentId,
      batch: req.batch._id,
    }).lean();

    if (!enrollmentRecord) {
      enrollmentRecord = await enrollmentProgressModel
    ...
    if (!enrollmentRecord) {
      return next(
        httpErrors.BadRequest(
          "Student is not enrolled. Please enroll the student first before creating a live class."
        )
      );
    }
...
const createCronJobHandlerController = async (req, res, next) => {
  try {
    ...
    let codeValue = "Cron@SummitScholars123";
