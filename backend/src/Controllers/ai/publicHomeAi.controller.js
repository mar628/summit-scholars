...
const publicHomeAiAgentController = async (req, res, next) => {
  try {
    ...
    const finalPrompt = `
      You are an AI teaching assistant for Summit Scholars Hub, an educational platform dedicated to 
    ...
    const config = {
      headers: {
        ...
        "X-Title": "Summit Scholars Hub",
        Referer: "https://www.summitscholars.co.ke",
      },
    };
    const json = {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an AI teaching assistant for Summit Scholars Hub, focused on providing high-quality educational content.",
        },
        {
          role: "user",
          content: [
