...
const { ADMIN, STUDENT, TEACHER } = require("../../Constants/roles.constants");
...
const MyProfileController = async (req, res, next) => {
  try {
    ...
    let data = cacheData; // use cached data if available

    if (data) {
      // cache hit - use cacheData
    } else {
      let selectData =
...
const RegisterTeacherController = async (req, res, next) => {
  try {
    logger.info("Controller-user.controller-RegisterTeacherController-Start");
    const { email } = req.body;
    const userExist = await userModel.findOne({ email });
    if (userExist) return next(httpErrors.BadRequest(USER_CONSTANTS.USER_ALREADY_EXISTS));
    ...
    const newTeacher = new userModel(details);
    await newTeacher.save();

    logger.info("Controller-user.controller-RegisterTeacherController-End");
    res.status(201).send({
      success: true,
      statusCode: 201,
    ...
    logger.error("Controller-user.controller-RegisterTeacherController-Error", error);
    errorHandling.handleCustomErrorService(error, next);
  }
};
...
const GetTeachersController = async (req, res, next) => {
  try {
    const docs = await userModel.find({ role: TEACHER }).select("-password -google").sort({ createdAt: -1 }).lean();
    res.status(200).send({ success: true, statusCode: 200, data: docs });
  } catch (error) {
    errorHandling.handleCustomErrorService(error, next);
  }
};
